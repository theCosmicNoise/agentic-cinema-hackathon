...
# The ledger has to outlive an instance. Cloud Run's /tmp does not, so state
# goes to a bucket; without one, a cold start silently empties the ledger.
BUCKET="${PROJECT}-clearcut-state"
echo "==> Ensuring state bucket gs://${BUCKET}"
gcloud storage buckets create "gs://${BUCKET}" --project "${PROJECT}" \
  --location "${REGION}" --uniform-bucket-level-access >/dev/null 2>&1 || true
gcloud storage buckets add-iam-policy-binding "gs://${BUCKET}" \
  --member="serviceAccount:${RUNTIME_SA}" \
  --role=roles/storage.objectAdmin --project "${PROJECT}" >/dev/null 2>&1 || true
...
gcloud run deploy "${SERVICE}" \
  ...
  --concurrency 8 \
  --max-instances 4 \
  --set-env-vars "GOOGLE_CLOUD_PROJECT=${PROJECT},GOOGLE_CLOUD_LOCATION=${REGION},GOOGLE_GENAI_USE_VERTEXAI=TRUE,CLEARCUT_DATA_DIR=/tmp/clearcut,CLEARCUT_BUCKET=${BUCKET}" \
